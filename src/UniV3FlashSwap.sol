...
abstract contract UniV3FlashSwap is IUniswapV3FlashCallback, RepaimentStorage{
    
    using BytesLib for bytes;
    using SafeERC20 for IERC20;

    uint256 private constant ONEFLASHSWAPUNIV3 = 84;
    address private constant FACTORYV3 = 0x1F98431c8aD98523631AE4a59f267346ea31F984;
    ...
    // should be overriden if you want to use other pools
    function initializeKnownPools() internal virtual {
        ...
        numberOfPoolForAsset[WETH] = 5;
        numberOfPoolForAsset[DAI] = 2;
        numberOfPoolForAsset[USDC] = 6;
        numberOfPoolForAsset[USDT] = 3;

        poolsForAsset[WETH][1] = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640; // USDCWETH005
        poolsForAsset[WETH][2] = 0x4585FE77225b41b697C938B018E2Ac67Ac5a20c0; // WBTCWETH005
        poolsForAsset[WETH][3] = 0x8ad599c3A0ff1De082011EFDDc58f1908eb6e6D8; // USDCWETH03
        poolsForAsset[WETH][4] = 0xCBCdF9626bC03E24f779434178A73a0B4bad62eD; // WBTCWETH03
        poolsForAsset[WETH][5] = 0x4e68Ccd3E89f51C3074ca5072bbAC773960dFa36; // WETHUSDT03

        poolsForAsset[DAI][1] = 0x5777d92f208679DB4b9778590Fa3CAB3aC9e2168; // DAIUSDC001
        poolsForAsset[DAI][2] = 0x6c6Bc977E13Df9b0de53b251522280BB72383700; // DAIUSDC005

        poolsForAsset[USDC][1] = 0x5777d92f208679DB4b9778590Fa3CAB3aC9e2168; // DAIUSDC001
        poolsForAsset[USDC][2] = 0x3416cF6C708Da44DB2624D63ea0AAef7113527C6; // USDCUSDT001
        poolsForAsset[USDC][3] = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640; // USDCWETH005
        poolsForAsset[USDC][4] = 0x6c6Bc977E13Df9b0de53b251522280BB72383700; // DAIUSDC005
        poolsForAsset[USDC][5] = 0xc63B0708E2F7e69CB8A1df0e1389A98C35A76D52; // FRAXUSDC005
        poolsForAsset[USDC][6] = 0x8ad599c3A0ff1De082011EFDDc58f1908eb6e6D8; // USDCWETH03

        poolsForAsset[USDT][1] = 0x3416cF6C708Da44DB2624D63ea0AAef7113527C6; // USDCUSDT001
        poolsForAsset[USDT][2] = 0x11b815efB8f581194ae79006d24E0d814B7697F6; // WETHUSDT005
        poolsForAsset[USDT][3] = 0x4e68Ccd3E89f51C3074ca5072bbAC773960dFa36; // WETHUSDT03
    }
    ...
    function maxFlashSwap(address token) public view virtual returns(uint256 amount){
        ...
        for (uint256 i = 1; i <= numberOfPoolForAsset[token]; i++){
            amount += IERC20(token).balanceOf(poolsForAsset[token][i]);
        }
    }
    ...
    function estimateFlashSwapFee(address token, uint256 amount) public view virtual returns(uint256 feesEstimated){
        ...
        while (flashSwapPath.length > 0){
            ...
            feesEstimated += _amount * fee / 1e6 + 1;
    ...
    function uniswapV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data) external virtual override {
        ...
        if (fee0 > 0){
            ...
            _updateRepaimentStorage(token0, pool, amount0, fee0);
        }
        if (fee1 > 0){
            address token1 = IUniswapV3Pool(pool).token1();
            _updateRepaimentStorage(token1, pool, amount1, fee1);
        }
        ...
        for (uint256 i = 0; i < borrowedTokens.length; i++){
            ...
            for (uint256 j; j < tokenLoans.length; j++){
                ...
                require(
                    ...
                    "UniV3FlashSwap: repay loan: not enough balance to repay loan"
                );
                feeTotal += loan.fee;
    ...
    function _buildFlashSwapsArray(
        ...
        require(tokens.length == amounts.length, "UniV3FlashSwap: length of amounts and tokens doesnt match");
        ...
        for (uint256 i = 0; i < tokens.length; i++){
            ...
            require(numberOfPoolForAsset[token] > 0, "UniV3FlashSwap: asset not supported");
            require(amount <= maxFlashSwap(token), "UniV3FlashSwap: amount to fs > maxFlashSwap");
            ...
            while (tempAmount > 0) {
                ...
                address nextPool = poolsForAsset[token][counter + 1];
                uint256 nextPoolBalance = IERC20(token).balanceOf(nextPool);
                uint256 increment = nextPoolBalance >= tempAmount? tempAmount: nextPoolBalance;
                uint256 amount0 = IUniswapV3Pool(nextPool).token0() == token? increment : 0;
                uint256 amount1 = amount0 > 0? 0 : increment;

                if (flashSwapPath.length == 0){
                    flashSwapPath = abi.encodePacked(nextPool, amount0, amount1);
                    continue;
                }
                ...
                assembly {
                    ...
                    let numberOfFlashSwaps := div(mload(flashSwapPath), ONEFLASHSWAPUNIV3)
                    ...
                    for {let j := 0} lt(j, numberOfFlashSwaps) {j:= add(j, 1)}{
                        let swapptr := add(startPtr, mul(j, ONEFLASHSWAPUNIV3))
    ...
    function _executeFlashSwaps(bytes memory flashSwapPath) private {
        ...
        if (flashSwapPath.length > 0) {
                (address pool, uint256 amount0, uint256 amount1) = decodeFirstFlashSwap(flashSwapPath);
                IUniswapV3Pool(pool).flash(address(this), amount0, amount1, flashSwapPath);
        } 
    ...
    function skipFlashSwap(bytes memory flashSwapPath) private pure returns (bytes memory){
        return flashSwapPath.slice(ONEFLASHSWAPUNIV3, flashSwapPath.length - ONEFLASHSWAPUNIV3);
    }

    // should be overriden with your own logic 
    function onUniV3FlashSwap() internal virtual;
}
